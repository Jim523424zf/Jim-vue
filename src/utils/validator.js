...
export function testPassword (rule, value, callback) {
    ...
    if (value == '' || value == undefined || value == null) {
        ...
        if (!regex.test(value)) {
            callback(new Error('请输入正确密码格式(8-16为英文字母+数字组合)'))
        } else {
            callback()
        }
    }
...
export function validateURL (rule, value, callback) {
    // /http[s]{0,1}:\/\/([\w.]+\/?)\S*/
    const reg = /http{0,1}:\/\/([\w.]+\/?)\S*/
    if (value == '' || value == undefined || value == null) {
        callback()
    } else {
        if (!reg.test(value)) {
            callback(new Error('必须是http://开头'))
        } else {
            callback()
        }
    }
...
// address: [{ required: true, message: '展示地址必须填写', trigger: 'change' }, { validator: validateURL, trigger: 'change' }],
